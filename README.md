...
## （8）epoll+reactor和io_uring的区别

| **特性**     | **Muduo / Epoll**                                            | **你的 io_uring 项目**                                       |
| ------------ | ------------------------------------------------------------ | ------------------------------------------------------------ |
| **核心模式** | **Reactor (反应堆)**                                         | **Proactor (前摄器)**                                        |
| **逻辑流程** | **就绪通知**：Epoll 告诉你“数据来了”，**你自己调用 `read` 去读**。 | **完成通知**：你告诉内核“读到 buffer 里”，内核读完告诉你**“读好了”**。 |
| **系统调用** | `read`/`write` 也是系统调用，会有开销。                      | 读写操作在 `submit` 时批量提交，**0 系统调用** (SQPOLL模式)。 |
| **内存拷贝** | 内核 -> 用户态 (即使是 Zero Copy 也有局限)。                 | 更加彻底的 Zero Copy 支持。                                  |
| **编程难度** | 中等（处理 EAGAIN, 拆包）。                                  | **极高**（要管理 buffer 生命周期，异步逻辑复杂）。           |
